...
import com.ruddersoft.videoplayer.databinding.ActivityExoPlayerBinding
import com.ruddersoft.videoplayer.utils.PlayerGestureHelper
import com.ruddersoft.videoplayer.utils.toMillis
...
class ExoPlayerActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Handle display cutouts for notched devices
        ...
        // SeekBar listener for scrubbing
    ...
    override fun onStart() {
        ...
        // Get the video URL from the intent (passed from VideoActivity)
        val videoUrl = intent.getStringExtra("VIDEO_URL")

        if (videoUrl != null) {
            initializePlayerView()
            playVideo(Uri.parse(videoUrl)) // Parse the videoUrl as a URI and play it
        } else {
    ...
    private fun playVideo(uri: Uri) = lifecycleScope.launch(Dispatchers.IO) {
        try {
            ...
            Log.e("ExoPlayerActivity", "Error playing video", e)
            finish()
        }
    }
    ...
    private fun initializePlayerView() {
        ...
        audioTrackButton.setOnClickListener {
            ...
            TrackSelectionDialogFragment(
                ...
                onTrackSelected = { trackIndex -> player.switchTrack(C.TRACK_TYPE_AUDIO, trackIndex) }
            ).show(supportFragmentManager, "AudioTrackSelectionDialog")
        }

        nextButton.setOnClickListener {
            val newPosition = player.currentPosition + seekIncrement.toMillis
            player.seekForward(newPosition.coerceAtMost(player.duration), shouldFastSeek)
        }

        prevButton.setOnClickListener {
            val newPosition = player.currentPosition - seekIncrement.toMillis
            player.seekBack(newPosition.coerceAtLeast(0), shouldFastSeek)
        }
        ...
        videoZoomButton.setOnLongClickListener {
            VideoZoomOptionsDialogFragment(
                currentVideoZoom = currentZoomMode,
                onVideoZoomOptionSelected = { applyVideoZoom(showInfo = true) }
